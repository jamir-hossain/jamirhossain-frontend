         ...
         <div className='row'>
            ...
            <div className='card col-10 col-sm-10 col-md-6 CardBox cardBg cardStyle'>
               ...
               <form className='mt-3' onSubmit={handleSubmit(onSubmit)}>
                  ...
                  <div className='form-group'>
                     <input className='form-control' placeholder='Message Subject' name="subject" ref={register({ required: true })} />
                     {errors.subject && <span className='text-danger'>Subject is required</span>}
                  </div>

                  <div className='form-group'>
                     <textarea className='form-control' placeholder='Your Message' rows='4' name="text" ref={register({ required: true })} />
                     {errors.text && <span className='text-danger'>Message Body is required</span>}
                  </div>
                  
                  <div className='text-center submitBtn'>
                     {isSuccess &&
                        <p className={result.error ? "successMsg text-danger" : "successMsg text-success"}> {result.error ? result.error : result.success && result.success} </p>
                     }
                     <Button type="submit" className="button px-5" variant="contained" color="primary" endIcon={<Send/>}>Send</Button>
                  </div>
               </form>
